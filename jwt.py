...
# ─────────────────────────────────────────
TOKEN         = "8712613654:AAHyZX0OpaIMmnlp7HBXUvpayBKpdAisYPo"       # Paste your bot token
ADMIN_ID_RAW  = 8311041202                  # Your Telegram user ID
ADMIN_CONTACT = "https://t.me/sahil_exeo"     # Your contact link
# ─────────────────────────────────────────
...
async def start(update: Update, context: CallbackContext) -> None:
    ...
    start_msg = (
        ...
        f"      👋 *Welcome, {username}!*\n"
        f"╚══════════════════════╝\n\n"
        f"🚀 *JWT Token Generator Bot*\n"
        f"👨‍💻 *Developer:* [SAHIL](https://t.me/sahil_exeo)\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"📁 *How to use:*\n"
        f"Send a `.json` file with account credentials:\n"
        f"```\n"
        f'[\n  {{"uid": "user1", "password": "pass1"}},\n  {{"uid": "user2", "password": "pass2"}}\n]\n'
        f"```\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📂 *Output Files:*\n"
        f"✅ `jwt_token.json` — Successful tokens\n"
        f"✅ `accountsREGION.json` — Region sorted\n"
        f"✔️ `working_account.json` — Valid UID/Pass\n"
        f"❌ `lost_account.json` — Failed entries\n\n"
        f"⚠️ *Max file size:* `{MAX_FILE_SIZE / 1024 / 1024:.1f} MB`\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"👑 *VIP Features:*\n"
        f"  ⚡ Auto-upload tokens to GitHub\n"
        f"  🔁 Schedule files for periodic processing\n\n"
        f"📌 Tap *Help* below to see all commands."
    )
...
async def help_command(update: Update, context: CallbackContext) -> None:
    ...
    help_text = (
        ...
        "  `/vipshop` — View VIP plans & pricing\n"
        "  `/cancel` — Cancel any active operation\n\n"
        "━━━━ 🔧 *VIP Commands* ━━━━\n"
        "  `/setgithub <TOKEN> <owner/repo> <branch> <file.json>`\n"
        "  `/mygithub` — View your GitHub config\n"
        "  `/setfile <Interval> <Name.json>` — Schedule auto-processing\n"
        "     📏 *Format:* `Xm` mins · `Xh` hours · `Xd` days _(min: 5m)_\n"
        "  `/removefile <Name.json>` — Remove a scheduled file\n"
        "  `/scheduledfiles` — List all your schedules\n\n"
        "━━━━ 👑 *Owner / Admin Commands* ━━━━\n"
        "  `/vip add <user_id> <days>` — Grant VIP\n"
        "  `/vip remove <user_id>` — Revoke VIP & configs\n"
        "  `/vip list` — View all active VIPs\n"
        "  `/broadcast <msg>` — Message all users\n\n"
        "━━━━ 📤 *Manual Processing* ━━━━\n"
        "  1️⃣ Send a JSON file with UID + password pairs\n"
        "  2️⃣ Bot processes & returns result files\n"
        "  3️⃣ VIP users get `jwt_token.json` auto-uploaded\n\n"
        "━━━━ ⚙️ *Auto Processing (VIP)* ━━━━\n"
        "  1️⃣ Use `/setfile` to define schedule & name\n"
        "  2️⃣ Send the JSON file when prompted\n"
        "  3️⃣ Bot runs it automatically at set intervals\n"
    )
    await update.message.reply_text(
        help_text,
        parse_mode=ParseMode.MARKDOWN,
...
async def vip_shop_command(update: Update, context: CallbackContext) -> None:
    ...
    vip_shop_text = (
        ...
        "━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "📩 *To Purchase:*\n"
        f"👉 Contact [Admin Here]({ADMIN_CONTACT_LINK})"
    )
...
async def vip_status_command(update: Update, context: CallbackContext) -> None:
    ...
    if expiry_date_str and "Invalid" not in expiry_date_str:
        ...
        status_msg = (
            "⚠️ *VIP Status Error*\n\n"
            "Could not read your expiry date.\n"
        ...
        status_msg = (
            ...
            "       📇 *VIP Status*\n"
            "╚══════════════════════╝\n\n"
            "ℹ️ *Status:* `Regular User`\n\n"
            "🚀 Upgrade to *VIP* to unlock:\n"
            "  ⚡ Auto GitHub uploads\n"
            "  🔁 Scheduled file processing\n\n"
            "👉 Tap *VIP Shop* to see plans!"
        )
...
async def main() -> None:
    ...
    logger.info("Using embedded JWT generator (OB54)")
    ...
    print(" 👨‍💻 Developer: SAHIL (@sahil_exeo)")
